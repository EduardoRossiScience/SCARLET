function [output_st] = SCARLET_v1_GMDplotFig11_12(cored_struct, N_particles, ...
...
%     IMPORTANT! This version has been modified in order to generate the
%     figures according to the GMD editorial standard
...
%__________________________________________________________________________
    ...
    % N_particles: number of particle to stick around the central core
    % core_size: size of the core
    % size_min: minimum particle size 
    % size_max: maximum particle size 
    % cone: solid angle of investigation
    % rays: number of rays per solid angle
    % euler: number of rotations per ray
...
%   oooooooo   ooooo oo     oo  oo   ooo  oooooooo oooooooo     ooo
    ...
    % Coating particles
    closet.N_particles = N_particles;                                                % Number of the coating particles (core not included)
    closet.vector_coat_sizes_mu = round(getRandomNumberVec(size_min, size_max, closet.N_particles));
