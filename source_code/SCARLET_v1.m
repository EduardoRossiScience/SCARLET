function [output_st] = SCARLET_v1(cored_struct)
    
%     SCARLET: SpheriCal Approximation for viRtuaL aggrEgaTes 
%     Copyright (c) 2020, Eduardo Rossi & Costanza Bonadonna
...
%__________________________________________________________________________
    ...
    % Please refer to the user guide for any additional information
    
    
    % Credits and copyrights for some of the functions or subroutines used
    % in the code and comprised in the Matlab R2015b release
...
%   oooooooo   ooooo    ooo     oooooo    oo       oooooooo  ooooooooo
%   ooo       oo       ooooo    oo   ooo  oo       oo       ooooooooo
%   oo        oo      oo   oo   oo   ooo  oo       oo          ooo
%   oooooooo  oo     oo     oo  oooooo    oo       oooooooo     ooo
%        ooo  oo     ooooooooo  oooo      oo       oo          ooo
%         oo  oo     oo     oo  oo ooo    oooooooo oo          ooo
%   oooooooo   ooooo oo     oo  oo   ooo  oooooooo oooooooo     ooo
    ...
    closet.core_size_mu = 100;                                             % Core size
    
    % Density of the core (kg/m^3)
    closet.core_density = 2500;  
    
    % Coating particles
    closet.N_particles = 10;                                                % Number of the coating particles (core not included)
    closet.vector_coat_sizes_mu = round(getRandomNumberVec(50, 50, closet.N_particles));
    closet.displace_in_order = 0;                                          % Boolean variable to decide if the coating is random (0) or not (1)
    
    % Cone of inspection features
    closet.cone_aperture_degree = 180;                                     % Cone aperture of inspection: [deg]
    closet.N_raysXSA = 10;                                                  % Number of rays per each solid angle
    
    % Euler angles 
    closet.N_Euler_triplets = 10;                                           % Number of angles per fixed direction within the solid angle
    ...
    closet.delta = 0.05;                                                   % Constant for the inward movement (coarse)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%% Producing the STL file of the composed aggregate %%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    
    buio = change2UniqueObjectFinal(output_st);
    buio.fv
    stlwrite('SCARLET_new_stl.stl', buio.fv);
...
function [front_points, g_Object, closet, output] = mainFunction(closet, cored_struct)
    ...
    [closet]  = initializeEmptyMatrices(closet);
     ...
     % Loop over each single particle to be placed around the inner core
     for id_particle = 1:closet.N_particles
         ...
         for id_single_ray = 1:closet.N_raysXSA
             ...
             if closet.displace_in_order
                order_RND_yn = id_particle;
             else
                order_RND_yn = randperm(closet.n_obj_coat);
             end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [output, C, DT_external, front_points] = calculatePorosity(closet, s_Blob, g_Object);
...
function [output, C, DT_external, front_points] = calculatePorosity(closet, s_Blob, g_Object)
